...
namespace SAE401_API_Vinted.Controllers.Tests
{
    ...
    public class AvisControllerTests
    {
        ...
        public void Initialize()
        {
            ...
            {
                AvisId = 1,
                AcheteurId = context.Vinties.Where(v => v.VintieId == 1).FirstOrDefault().VintieId,
                VendeurId = context.Vinties.Where(v => v.VintieId == 32).FirstOrDefault().VintieId,
                CodeTypeAvis = context.TypesAvis.Where(v => v.TypeAvisID == 1).FirstOrDefault().TypeAvisID,
                Commentaire = "Excellente vente",
                Note = 5,
                APourAcheteur = context.Vinties.Where(v => v.VintieId == 1).FirstOrDefault().VintieId,
            };
        ...
        public void PostAvis_ModelNotValidated_CreationNotOk()
        {
            ...
            Assert.AreEqual(badRequestResult.StatusCode, StatusCodes.Status400BadRequest, "Result ne retourne pas 400 bad request");
        ...
        [TestMethod()]
        public void PutAvis_ValidUpdate_ReturnsNoContent()
        {
            //Act
            var result = controller.PutAvis(1, avisTestExist).Result;

            //Assert
            var avisToGet = context.Avis.Where(a => a.AvisId == 1).FirstOrDefault();
