...
namespace SAE401_API_Vinted.Controllers.Tests
{
    ...
    public class AvisControllerTests
    {
        ...
        public void GetTypeAvisByIdTest_Correct()
        {
            ...
            //Act
            var result = controller.GetTypeAvisById(1).Result;

            //Assert
            Assert.IsNotNull(result, "TypeAvis non retourné");
            Assert.IsInstanceOfType(result, typeof(ActionResult<TypeAvis>), "Result n'est pas un action result");
            Assert.AreEqual(result.Value, typeAvis, "Les typeAvis ne sont pas égaux");
        }
        ...
        public void GetTypeAvisByIdTest_UnkownId()
        {
            ...
            //Act
            var result = controller.GetTypeAvisById(4273).Result;
        ...
        public void PostAvis_ModelValidated_CreationOk()
        {
            ...
            //Assert
            Avis avisToGet = context.Avis.Where(a => a.AvisId == 7342).FirstOrDefault();

            Assert.IsInstanceOfType(result, typeof(ActionResult<Avis>), "Result n'est pas un action result");
            Assert.IsInstanceOfType(result.Result, typeof(CreatedAtActionResult), "Result n'est pas un CreatedAtActionResult");
            Assert.AreEqual(avisToGet, avisTestNotExist, "Les adresses ne sont pas identiques");
        ...
        public void PostAvis_ModelNotValidated_CreationNotOk()
        {
            ...
            var result = controller.PostAvis(avisTestInvalid).Result;

            //Assert
            Avis avisToGet = context.Avis.Where(a => a.AvisId == 7342).FirstOrDefault();

            Assert.IsInstanceOfType(actionResult, typeof(ActionResult<Avis>), "Pas un ActionResult<Utilisateur>");
